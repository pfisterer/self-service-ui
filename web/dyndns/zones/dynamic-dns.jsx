    ...
    return (
        <Paper withBorder radius="md" p="md">
            <SimpleGrid cols={{ base: 1, sm: 3 }} spacing="md" verticalSpacing="sm">
                <InfoItem label={t('dyndns.prefilled.nameserver')} value={`${cfg.host}:${cfg.port}`} />
                <InfoItem label={t('dyndns.prefilled.zone')} value={cfg.zoneNoDot} />
                <InfoItem label={t('dyndns.prefilled.tsigKey')} value={cfg.keyname} note={cfg.alg} />
            </SimpleGrid>
        </Paper>
    );
}
...
function RecordNameInput({ value, onChange, fqdnPreview }) {
    ...
    return (
        <TextInput
            label={t('dyndns.recordName.label')}
            description={fqdnPreview
                ? <Trans i18nKey="dyndns.recordName.descriptionWithPreview" values={{ fqdn: fqdnPreview }}
                    components={{ 1: <code />, 2: <code />, 3: <code /> }} />
                : <Trans i18nKey="dyndns.recordName.description"
                    components={{ 1: <code />, 2: <code /> }} />}
            placeholder="my-host"
            value={value}
            onChange={e => onChange(e.currentTarget.value)}
            error={value.trim() ? recordNameError(value, t) : null}
            size="xs"
            maw={460}
        />
    );
    ...
    return (
        <Stack gap="lg">
            <Text size="sm" c="dimmed">
                <Trans i18nKey="dyndns.oneShot.intro" components={{
                    1: <code />,
                    2: <ExternalLink href="https://datatracker.ietf.org/doc/html/rfc2136" />,
                    3: <code />,
                }} />
            </Text>
            ...
            <Paper withBorder radius="md" p="md">
                <Grid>
                    <Grid.Col span={{ base: 12, sm: 6 }}>
                        <TextInput label={t('dyndns.oneShot.name')} name="name" value={form.name} onChange={handleChange}
                            error={recordNameError(form.name, t)} />
                    </Grid.Col>
                    <Grid.Col span={{ base: 12, sm: 3 }}>
                        <Select label={t('dyndns.oneShot.type')} name="type" value={form.type}
                            onChange={(val) => setForm(prev => ({ ...prev, type: val }))}
                            data={['A', 'AAAA', 'CNAME', 'TXT', 'MX']} />
                    </Grid.Col>
                    <Grid.Col span={{ base: 12, sm: 3 }}>
                        <NumberInput label={t('dyndns.oneShot.ttl')} name="ttl" min={0} value={form.ttl}
                            onChange={(val) => setForm(prev => ({ ...prev, ttl: val }))} />
                    </Grid.Col>
                    <Grid.Col span={12}>
                        <TextInput label={t('dyndns.oneShot.value')} name="value" value={form.value} onChange={handleChange}
                            error={form.value.trim() ? recordValueError(form.type, form.value, t) : null} />
                    </Grid.Col>
                </Grid>
            </Paper>
            ...
            {zone.zone_keys.map(key => (
                <Paper key={key.keyname} withBorder radius="md" p="md">
                    <Title order={5} mb="sm">{t('dyndns.oneShot.keyHeading', { keyname: key.keyname })}</Title>
            ...
            <Alert variant="light" color="blue" icon={<AlertCircle size="16" />} title={t('dyndns.oneShot.autoTitle')}>
                <Text size="sm">
                    <Trans i18nKey="dyndns.oneShot.autoBody" components={{
                        1: <ExternalLink href="https://wiki.archlinux.org/title/Systemd/Timers" />,
                        2: <b />,
                    }} />
                </Text>
            </Alert>
...
${fqdnNoDot}`;
    ...
    return (
        <Stack gap="md">
            <Text size="sm" c="dimmed">
                <Trans i18nKey="dyndns.ddclient.intro" components={{
...
export function DynamicDns({ zone }) {
    ...
    return (
        <Stack gap="lg">
            <TabIntro title={t('dyndns.dynamicDns.title', { zone: cfg.zoneNoDot })}>
                <Trans i18nKey="dyndns.dynamicDns.intro" components={{
                    1: <ExternalLink href="https://datatracker.ietf.org/doc/html/rfc2136" />,
                    2: <code />,
                    3: <ExternalLink href="https://ddclient.net/" />,
                }} />
            </TabIntro>
            ...
            <Accordion variant="separated">
                <Accordion.Item value="oneshot">
                    <Accordion.Control>{t('dyndns.dynamicDns.accordionOneShot')}</Accordion.Control>
                    <Accordion.Panel><OneShotPanel zone={zone} /></Accordion.Panel>
                </Accordion.Item>

                <Accordion.Item value="ddclient">
                    <Accordion.Control>{t('dyndns.dynamicDns.accordionDdclient')}</Accordion.Control>
