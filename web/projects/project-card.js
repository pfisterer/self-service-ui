...
import { AlertTriangle, Eye, History, Rocket, Users, X } from 'lucide-preact';
import { useDisclosure } from '@mantine/hooks';
import { Alert, Badge, Box, Button, Card, Group, Select, Stack, Text, Tooltip } from '@mantine/core';
import { ProjectChangesDiff, ProjectBar } from './component-common.js';
import { RequestHistoryModal } from './project-history-modal.js';
import { RequestModal } from './project-modal.js';
import { ProjectRejectModal } from './project-reject-modal.js';
import { isReadOnly, statusStyle, statusLabel, UNLIMITED_QUOTA } from './util-project.js';
...
function FunderImpactPanel({ config, funder, incomingQuota }) {
    ...
    return html`
        <${Stack} gap="xs">
            <${Text} size="xs" c="dimmed">Impact after granting:<//>
            ${config.projects.map(r => html`
                <${ProjectBar}
                    key=${r.id}
                    resource=${r}
        ...
        <${Card} withBorder shadow="sm" radius="md" style=${{ display: 'flex', flexDirection: 'column', height: '100%' }}>
            <${Box} style=${{ flex: 1 }}>
                ...
                <${Group} justify="space-between" mb="xs">
                    ...
                    <${Text} size="xs" c="dimmed">${createdDate}<//>
                <//>
                ...
                <${Stack} gap="3" mb="xs">
                    ...
                    ${isOSOnly && (() => {
                        const owners = (req.requester_tokens || [])
                            ...
                            .map(t => t.slice(5));
                        return owners.length > 0 && html`
                            <${Group} gap="xs" wrap="wrap">
                                <${Text} size="xs" c="dimmed" style=${{ minWidth: 68 }}>Owner:<//>
                                ${owners.map(email => html`
                    ...
                    ${!isOSOnly && (req.requester_tokens || []).length > 0 && html`
                        <${Group} gap="xs" wrap="wrap">
                            <${Text} size="xs" c="dimmed" style=${{ minWidth: 68 }}>Requester:<//>
                            ${(req.requester_tokens || []).map(t => html`
                    ...
                    ${resourceSummary && html`
                        <${Group} gap="xs">
                            <${Text} size="xs" c="dimmed" style=${{ minWidth: 68 }}>Resources:<//>
                            <${Text} size="xs">${resourceSummary}<//>
                        <//>
                    `}

                    <${Group} gap="xs" wrap="wrap">
                        ${req.termination_date && html`
                            <${Badge} size="xs" variant="outline" color="gray">
                                Expires ${new Date(req.termination_date).toLocaleDateString()}
                            <//>
                        `}
                        ${authorizedCount > 0 && html`
                            <${Badge} size="xs" variant="outline" color="gray">
                                <${Users} size="10" style=${{ marginRight: 3, verticalAlign: 'middle' }} />
                                ${authorizedCount} user${authorizedCount !== 1 ? 's' : ''}
                            <//>
                        `}
                        ${isApproved && req.funded_by && html`
            ...
            ${!readOnly && isAdminView && isPendingApproval && html`
                <${Card.Section} withBorder inheritPadding py="xs">
                    <${Text} size="xs" c="dimmed" mb="xs">Grant funding from:<//>
                    <${Select}
                        size="sm"
            ...
            <${Card.Section} withBorder inheritPadding py="xs" mt="auto">
                <${Group} grow>
                    ...
                    ${readOnly && isAdminView && onPromote && !isPendingPromotion && html`
                    ...
                    `}
                    ${!readOnly && (isApproved || isChangeRejected || isRejected) && onRelease && html`
            ...
            ${readOnly && isAdminView && onPromote && html`
                <${RequestModal}
