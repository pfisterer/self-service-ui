...
import { AlertCircle, ArrowRight, Check, FileText, LogOut, X } from 'lucide-preact';
import { Badge, Group, Modal, Paper, Text, Timeline } from '@mantine/core';
import dayjs from 'dayjs';
import relativeTime from 'dayjs/plugin/relativeTime';
import { ProjectChangesDiff, ProjectDisplay } from './component-common.js';
        ...
        <${Modal} opened=${opened} onClose=${onClose} title="History: ${request.reason}" size="lg">
            ${!request.history || request.history.length === 0 ? html`
                <${Paper} p="md" withBorder>
                    <${Group} gap="xs">
                        <${AlertCircle} size="18" />
                        <${Text}>No history available for this request.<//>
                    <//>
                <//>
            ` : html`
