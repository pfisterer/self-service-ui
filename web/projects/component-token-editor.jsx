...
export function TokenEditor({ label, description, error, rules = [], onAddRule = () => { }, onRemoveRule = () => { }, emptyMessage = 'No entries yet.' }) {
    ...
    return (
        <div>
            <Text fw={600} mb={description || error ? 'xs' : 'xs'}>{label}</Text>
            ...
            <Stack gap="sm">
                ...
                <Group align="flex-end" grow>
                    ...
                    <Button onClick={addRule} disabled={!tokenInput.trim()}>Add</Button>
                </Group>
                    ...
                    <Stack gap="xs">
                        ...
                        {(rules || []).map((rule) => (
                            <Group justify="space-between" align="center" gap="xs" wrap="nowrap"
                                ...
                                <Text style={{ textTransform: 'none', wordBreak: 'break-all' }}>{rule}</Text>

                                <Button variant="subtle" color="red" size="xs" compact px={4} style={{ flex: '0 0 auto' }}
                                        onClick={() => onRemoveRule(rule)}> x </Button>
                            </Group>
                        ))}
