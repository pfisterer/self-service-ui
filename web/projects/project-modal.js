...
import { ProjectChangesDiff, TerminationDatePicker } from './component-common.js';
import { TokenRoleEditor } from './component-token-role-editor.js';
import { normalizeArrayResponse, UNLIMITED_QUOTA } from './util-project.js';
...
function TabLabel({ label, hasError }) {
    return html`
        <${Group} gap="xs" wrap="nowrap">
            ${label}
            ${hasError && html`<${Badge} size="xs" color="red" circle>!<//>`}
        <//>
    `;
}
            ...
            <form onSubmit=${handleSubmit}>
                <${Stack}>
                    <${Tabs} value=${activeTab} onChange=${setActiveTab}>
                        ...
                        <${Tabs.Panel} value=${TAB_DETAILS}>
                            <${Stack}>
                                ...
                                ${!isPromote && !isEdit && !readOnly && html`
                                    <${FundFromSelect}
                        ...
                        <${Tabs.Panel} value=${TAB_ACCESS}>
                            <${Stack} gap="md">
                                <${TokenRoleEditor}
                                    ...
                                    readOnly=${readOnly}
                                />
                                ${isPromote && (promoteProject?.external_group_assignments?.length > 0) && html`
                                    <${Stack} gap="xs">
                                        ...
                                        <${Table} fz="xs" withRowBorders={false}>
                                            ...
                                            <${Table.Tbody}>
                                                ${promoteProject.external_group_assignments.map(eg => html`
                                                    <${Table.Tr}>
                                                        <${Table.Td}>
                                                            <${Text} size="xs" c="dimmed">
                                                                ${eg.group_name || eg.group_id}
                                                                ${eg.group_name && eg.group_name !== eg.group_id && html`
                                                                    <${Text} span size="xs" c="dimmed" ml="xs">(${eg.group_id})<//>
                                                                `}
                                                            <//>
                                                        <//>
                                                        <${Table.Td}>
                                                            <${Badge} size="xs" variant="light" color="gray">${eg.role}<//>
                                                        <//>
                                                    <//>
                                                `)}
                                            <//>
                    ...
                    <${Group} justify="flex-end" mt="md">
                        ${readOnly
                            ? html`<${Button} variant="default" type="button" onClick=${onClose}>Close<//>`
                            : html`
